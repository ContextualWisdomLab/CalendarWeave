...
| Calendar Resource Core v1 candidate is non-durable and below consumer parity | CalendarWeave #2 / ADR-0002 | Candidate application port covers collection plus strict VEVENT create/conditional-update/list/get, preserves RFC status, and orders authorization before parsing with 100% line/branch coverage; add durable concurrency-safe storage, external authorization admission, TZID/DURATION, and declared privacy/content semantics before release or consumer migration |
