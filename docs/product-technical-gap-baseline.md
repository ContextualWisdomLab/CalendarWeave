...
| Calendar Resource Core v1 candidate is non-durable and below consumer parity | CalendarWeave #2 / ADR-0002 | Candidate application port covers collection plus strict VEVENT create/list/get and preserves RFC status with 100% line/branch coverage; add durable concurrency-safe storage, authorization admission, conditional update, TZID/DURATION, and declared privacy/content semantics before release or consumer migration |
