...
| Calendar Resource Core candidates remain below consumer parity | CalendarWeave #2 / ADR-0002 / ADR-0003 / ADR-0004 | Stacked candidates cover strict VEVENT create/conditional-update/list/get, RFC status, bounded IANA TZID intervals, authorization-before-parse, and PostgreSQL restart/concurrency evidence with 100% line/branch coverage; add external authorization admission, operated backup/recovery evidence, VTIMEZONE/DURATION, and declared privacy/content semantics before release or consumer migration |
