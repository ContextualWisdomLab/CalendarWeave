...
Implement RFC behavior test-first. The current candidate profile accepts explicit `DTEND` intervals and, under ADR-0007, positive RFC 5545 `DURATION` intervals as mutually exclusive alternatives. Under ADR-0008, optional RFC 5545 `CLASS` is privacy intent rather than enforcement: omission defaults to `PUBLIC`, standard values are case-insensitive, and unknown registered/experimental token values fail-private. Unsupported recurrence, floating time, `VTIMEZONE`, provider and CalDAV capabilities must continue to fail closed until separately versioned and verified.
